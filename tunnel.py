...
def tunnel_config(cl=sys.argv[1:]):
    try:
        parser = argparse.ArgumentParser("tunnel config", description="Find local ports connecting to devices via ssh tunnels and write a config")
        ...
        if args.jump:
            ...
            rcl = ['ssh',args.jump,"tunnel.py"] + cl           
            ji = rcl.index("--jump")
            del rcl[ji]
            del rcl[ji]
            LOGGER.debug(rcl)
...
def tunnel_connect(cl=sys.argv[1:]):
    try:
        parser = argparse.ArgumentParser("tunnel connect", description="connect to a local port which is a tunnel to remote ssh hosts")
